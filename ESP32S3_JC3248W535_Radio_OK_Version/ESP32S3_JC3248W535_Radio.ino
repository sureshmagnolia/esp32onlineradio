...
// =============================================================================
// Zero-Stutter FreeRTOS Dedicated Audio Task (Pinned to Core 0)
// =============================================================================
void audioTask(void *pvParameters) {
    Serial.println("[AUDIO TASK] Dedicated Core 0 Task Running.");
    while (true) {
        ...
        if (isPlaying) {
            audio.loop();
            if (isBuffering && currentSource == SRC_RADIO) {
                if (audio.isRunning()) {
                    isBuffering = false;
                    currentError = ERR_NONE;
                    uiNeedsUpdate = true;
                } else if (millis() - streamStartTime > 12000) {
                    // Buffer Watchdog: 12 seconds elapsed with no stream
...
static esp_err_t http_root_handler(httpd_req_t *req) {
    ...
    const char* ptr = PAGE_INDEX;
    size_t remaining = strlen(PAGE_INDEX);
    while (remaining > 0) {
        size_t chunk_len = (remaining > 1024) ? 1024 : remaining;
        esp_err_t res = httpd_resp_send_chunk(req, ptr, chunk_len);
        if (res != ESP_OK) {
            Serial.printf("[HTTPD] / chunk send error: %d\n", res);
            return res;
        }
        ptr += chunk_len;
        remaining -= chunk_len;
    }
    esp_err_t res = httpd_resp_send_chunk(req, NULL, 0);
...
void setup() {
    ...
    Serial.printf("[SYSTEM] Free Heap: %d KB, Free PSRAM: %d KB (PSRAM Found: %s)\n", 
                  ESP.getFreeHeap() / 1024, ESP.getFreePsram() / 1024, psramFound() ? "YES" : "NO");
    ...
    // 10. Spawn Dedicated Audio Task on Core 0
