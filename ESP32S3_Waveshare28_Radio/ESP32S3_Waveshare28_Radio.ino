/**
 * =========================================================================================
 * Project: ESP32-S3 Waveshare 2.8" (320x240 IPS Landscape) - Ultimate Stereo Internet & SD Radio
 * 
 * Features:
 ...
 *   8. 🎧 Pure 2-Channel Hi-Fi Stereo Output via PCM5101 32-bit DAC (Dual Speakers)
 * =========================================================================================
 */
...
ButtonHandler bootButton(BUTTON_PIN_BOOT, true); // Active LOW with pull-up
void onPhysicalButtonEvent(ButtonEvent event);
...
void enterDeepSleep() {
    ...
    // 5. Configure RTC GPIO Pull-ups so INT line and BOOT button do not float LOW when CPU sleeps
    pinMode(TOUCH_PIN_NUM_INT, INPUT_PULLUP);
    pinMode(BUTTON_PIN_BOOT, INPUT_PULLUP);

    rtc_gpio_init((gpio_num_t)TOUCH_PIN_NUM_INT);
    rtc_gpio_set_direction((gpio_num_t)TOUCH_PIN_NUM_INT, RTC_GPIO_MODE_INPUT_ONLY);
    rtc_gpio_pullup_en((gpio_num_t)TOUCH_PIN_NUM_INT);
    rtc_gpio_pulldown_dis((gpio_num_t)TOUCH_PIN_NUM_INT);

    rtc_gpio_init((gpio_num_t)BUTTON_PIN_BOOT);
    rtc_gpio_set_direction((gpio_num_t)BUTTON_PIN_BOOT, RTC_GPIO_MODE_INPUT_ONLY);
    rtc_gpio_pullup_en((gpio_num_t)BUTTON_PIN_BOOT);
    rtc_gpio_pulldown_dis((gpio_num_t)BUTTON_PIN_BOOT);

    // Ensure touch and button are fully released before arming wakeup
    for (int i = 0; i < 20; i++) {
        if (digitalRead(TOUCH_PIN_NUM_INT) == HIGH && digitalRead(BUTTON_PIN_BOOT) == HIGH) break;
        delay(10);
    }

    // 6. Arm Capacitive Touch Interrupt (GPIO 4) & BOOT button (GPIO 0) as Wakeup Sources
    esp_sleep_enable_ext1_wakeup((1ULL << TOUCH_PIN_NUM_INT) | (1ULL << BUTTON_PIN_BOOT), ESP_EXT1_WAKEUP_ANY_LOW);
...
void openPowerOffPrompt() {
    ...
    lv_obj_set_size(modal_pwr_backdrop, 320, 240);
    ...
    lv_obj_set_size(modal_pwr_box, 290, 180);
...
void showWakeupPrompt() {
    ...
    lv_obj_set_size(modal_wake_backdrop, 320, 240);
...
void showTimerModal() {
    ...
    lv_obj_set_size(modal_timer_backdrop, 320, 240);
    ...
    lv_obj_set_size(modal_timer_box, 300, 210);
    ...
    lv_obj_set_size(pnl_time, 280, 46);
    ...
    lv_obj_set_size(pnl_dur, 280, 46);
...
void buildModernUI() {
    ...
    lv_obj_set_width(lbl_station_title, 200);
    ...
    lv_obj_set_width(lbl_stream_info, 200);
    ...
    lv_obj_set_size(btn_prev, 58, 38);
    lv_obj_set_pos(btn_prev, 2, 138);
    ...
    lv_obj_set_size(btn_play, 82, 38);
    lv_obj_set_pos(btn_play, 64, 138);
    ...
    lv_obj_set_size(btn_next, 58, 38);
    lv_obj_set_pos(btn_next, 150, 138);
    ...
    lv_obj_set_size(pnl_r, 92, 196);
    lv_obj_set_pos(pnl_r, 222, 2);
    ...
    lv_obj_set_size(slider_vol, 20, 105);
    ...
    lv_obj_set_size(btn_mute, 80, 32);
    ...
    lv_obj_set_size(btn_sleep, 40, 26);
lv_obj_add_flag(btn_sleep, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_set_size(pnl_clock_card, 200, 26);
lv_obj_add_flag(pnl_clock_card, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_set_size(list_categories, 96, 198);
    ...
    lv_obj_set_size(pnl_right_st, 212, 198);
    ...
    lv_obj_set_size(pnl_page_controls, 212, 32);
    ...
    lv_obj_set_size(btn_pprev, 60, 28);
    ...
    lv_obj_set_size(btn_pnext, 60, 28);
    ...
    lv_obj_set_size(list_stations, 212, 162);
    ...
    lv_obj_set_size(pnl_wifi_current, 310, 28);
    ...
    lv_obj_set_size(btn_scan, 110, 26);
    lv_obj_set_pos(btn_scan, 4, 32);
    ...
    lv_obj_set_pos(lbl_wifi_status, 120, 36);

    // Scanned Networks List
    list_wifi_networks = lv_list_create(tab_wifi);
    lv_obj_set_size(list_wifi_networks, 310, 134);
    ...
    lv_obj_set_size(box_wifi_connect, 310, 85);
    ...
    lv_obj_set_size(ta_wifi_pass, 170, 32);
    ...
    lv_obj_set_size(btn_do_conn, 64, 32);
    ...
    lv_obj_set_size(btn_cancel, 60, 32);
    ...
    lv_obj_set_size(kb_wifi, 310, 105);
    ...
    if (tab_sd_music) {
        ...
        lv_obj_set_size(pnl_sd_page_controls, 310, 32);
        ...
        lv_obj_set_size(btn_sd_shuffle, 100, 28);
        ...
        lv_obj_set_size(list_sd_tracks, 310, 162);
...
// =============================================================================
// Physical Hardware Button 1 (BOOT) Event Handler
// Single Touch: Next station / SD track
// Double Touch: Previous station / SD track
// Long Press / Hold: Continuous Volume Up ramping
// =============================================================================
void onPhysicalButtonEvent(ButtonEvent event) {
    switch (event) {
        case ButtonEvent::SINGLE_CLICK:
            Serial.println("[BUTTON] Single Touch -> Next Station / Track");
            if (currentSource == SRC_RADIO) {
                playStationByFilterIndex(currentFilterPosition + 1);
            } else {
                playNextSdTrack();
        ...
        case ButtonEvent::DOUBLE_CLICK:
            Serial.println("[BUTTON] Double Touch -> Previous Station / Track");
            if (currentSource == SRC_RADIO) {
                playStationByFilterIndex(currentFilterPosition - 1);
            } else {
                playPrevSdTrack();
        ...
        case ButtonEvent::HOLD_REPEAT:
            if (currentVolume < 21) {
                setSystemVolume(currentVolume + 1);
            }
            Serial.printf("[BUTTON] Hold -> Volume Up (%d/21)\n", currentVolume);
            break;

        case ButtonEvent::LONG_PRESS:
            Serial.println("[BUTTON] Hold -> Volume Ramp Starting");
            break;
...
void setup() {
    ...
    Serial.println("\n=======================================================");
    Serial.println("  ESP32-S3 Waveshare 2.8\" - Stereo Internet & SD Radio");
    Serial.println("=======================================================");
    if (psramInit()) {
    ...
    audio.setPinout(I2S_PIN_BCLK, I2S_PIN_LRCK, I2S_PIN_DOUT);
    audio.forceMono(false); // Enable Pure 2-Channel Hi-Fi Stereo Output
    Serial.println("[AUDIO] Configured pure 2-channel Hi-Fi Stereo mode for PCM5101!");
    bootButton.begin(onPhysicalButtonEvent);
    audio.setVolume(currentVolume);
